...
- [x] **MA-01** — MA is a selectable project state. MA projects show a state-gated "Download MA DLS Weekly Payroll" button on PayrollWeekDetailPage.
...
- [x] **SUB-01** — `subcontractors` table: `(id UUID, projectId FK→projects, name text NOT NULL, licenseNumber text, contactName text, contactEmail text, address text, createdAt text)`. Project-scoped. Migration + Drizzle schema.
...
- [x] **NFR-03** (continued) — All new routes apply `assertProjectAccess` before any data access.
