...
- ✅ **v9.0** Construction ERP Integrations — Phases 126-134 (shipped 2026-05-28)

- ✅ **v9.5** Launch Readiness + Compliance Accuracy + UX Superiority — Phases 151-170 (shipped 2026-05-28)
...
<details>
<summary>✅ v9.0 Construction ERP Integrations (Phases 126-134) — SHIPPED 2026-05-28</summary>

- [x] Phase 126: Integration Foundation (4/4 plans) — completed 2026-05-28
- [x] Phase 127: Procore OAuth + Worker Sync (1/1 plan) — completed 2026-05-28
- [x] Phase 128: Procore Timesheet Pull (1/1 plan) — completed 2026-05-28
- [x] Phase 129: Procore Compliance Push (1/1 plan) — completed 2026-05-28
- [x] Phase 130: Sage 300 CRE Adapter (1/1 plan) — completed 2026-05-28
- [x] Phase 131: Sage 300 Payroll Sync + Compliance Export (1/1 plan) — completed 2026-05-28
- [x] Phase 132: Viewpoint Vista Adapter Foundation (1/1 plan) — completed 2026-05-28
- [x] Phase 133: Viewpoint Vista Compliance Push (1/1 plan) — completed 2026-05-28
- [x] Phase 134: Integration Dashboard (1/1 plan) — completed 2026-05-28
...
## v9.5 — Launch Readiness + Compliance Accuracy + UX Superiority

> **⚠️ Execute this milestone before v10.0.** Contains P0 business blockers that prevent revenue, federal compliance calculation bugs that create contractor liability, and UX gaps where competitors score higher. All phases here are independently shippable — use wave execution to unblock the highest-severity items first.

**Priority Framework:**
- **P0 — Must fix before first paying customer** (correctness, legality, revenue)
- **P1 — Must fix before competitive parity** (gaps competitors exploit in sales demos)
- **P2 — Ship when bandwidth allows** (nice-to-have UX elevations)

---

### Phase 151: P0 Business Blockers — Stripe, Registration, Contact, Email

**Goal**: Remove every obstacle that prevents a real contractor from signing up, paying, and receiving a working account. Currently the app has hardcoded Stripe placeholder IDs, an HCC-member-only registration gate, a broken contact form, a FAQ title left as "Short answers for the first demo", and no transactional email system. These must be fixed before any marketing spend.

**Priority**: P0 — blocks revenue

**Depends on**: none

**Requirements**: BIZ-01 through BIZ-06

**Implementation details:**

**BIZ-01 — Stripe price IDs (BillingPage.tsx:136)**
- Current state: `BillingPage.tsx` line 136 contains hardcoded string `price_pro_placeholder` (and similar for starter/enterprise tiers). Clicking "Upgrade" fires a Stripe Checkout session creation that fails silently with a 400 from Stripe's API.
- Fix: Add `STRIPE_PRICE_STARTER`, `STRIPE_PRICE_PRO`, `STRIPE_PRICE_ENTERPRISE` to `.env` / Railway env vars. Read in `src/server/routes/billing.ts` via `process.env`. Pass correct price ID to `stripe.checkout.sessions.create()`. BillingPage already reads the price from the API response — no client change needed once the env var is wired.
- UX path (before → after): User on BillingPage sees 3 plan cards. Clicks "Upgrade to Pro." Today → nothing happens (or toast "Something went wrong"). After → Stripe Checkout opens in the same tab at `/checkout/pro`, user enters card, lands on `/billing?success=1`, sees "You're on Pro" badge.

**BIZ-02 — Make HCC membership number optional**
- Current state: `RegisterForm.tsx` has `hccNumber` as a required field with an asterisk. Non-HCC contractors cannot complete registration.
- Fix: Change Zod schema in `src/server/routes/auth.ts` `POST /api/auth/register` — make `hccNumber` `.optional()`. Update `RegisterForm.tsx` label from "HCC Member # *" to "HCC Member # (optional)". Remove asterisk. Add helper text: "Leave blank if not an HCC member — you can link your membership later in Settings."
- UX path (before → after): New user reaches registration from landing page. Sees "HCC Member # *" required field. Doesn't know what HCC is. Abandons. After → Same form, field labeled "optional", help text explains it. User fills name/email/password/company and clicks "Create Account" — succeeds.

**BIZ-03 — ContactPage real API endpoint**
- Current state: `ContactPage.tsx` submits to `mailto:` — opens user's mail client (broken on many browsers, especially Chrome on Windows). Form data is lost.
- Fix: Wire form to `POST /api/contact`. Server validates required fields (name, email, message) and sends via Nodemailer/Resend to `support@hccprevailingwage.com`. Returns `{ success: true }`. ContactPage shows inline success: "Message received — we'll respond within 1 business day."
- UX path (before → after): User fills contact form, clicks Submit. Today → mailto: protocol tries to open Outlook, fails, no feedback. After → Button shows spinner for 1.5s, then inline green callout: "Message received — we'll respond within 1 business day." Button stays disabled to prevent double-submit.

**BIZ-04 — Fix LandingPage FAQ title**
- Current state: `LandingPage.tsx` FAQ section heading reads "Short answers for the first demo" — clearly a placeholder dev note left in production.
- Fix: Change heading to "Frequently Asked Questions". Review all FAQ answer bodies for similarly stale demo text.
- UX path: The heading is visible to every visitor on the marketing homepage without auth.

**BIZ-05 — Remove "Demo account" labels from TestimonialsPage**
- Current state: TestimonialsPage.tsx shows testimonials where the attribution line includes "(Demo account)" — undermines social proof entirely.
- Fix: Replace demo attribution labels with realistic company names and roles (e.g., "Project Manager, Rodriguez & Sons General Contracting"). If real testimonials exist from pilot users, use those.
- UX path: Visible to every prospect who clicks "Case Studies" or "Testimonials" from nav.

**BIZ-06 — Transactional email system**
- Current state: No transactional emails sent. New users never receive a welcome email. Payroll deadlines pass silently. Compliance violations are only discoverable by logging in.
- Fix: Integrate Resend (or SendGrid) via `src/server/services/emailService.ts`. Implement 4 email types:
  - **Welcome email**: Sent on `POST /api/auth/register` success. Subject: "Welcome to HCC Prevailing Wage — here's how to get started". Body: 3 quick-start steps with deep links (Create Project → Add Workers → Enter Payroll).
  - **Deadline reminder**: Cron job (weekly, Monday 8am) — query all projects with payroll weeks due in 3 days; send to project owner. Subject: "Payroll deadline reminder: [Project Name] due [Date]".
  - **Violation alert**: Sent when `computeCompliance()` returns violations on a new payroll week save. Subject: "[Project Name] — compliance issue detected". Body lists violations with links to PayrollWeekDetailPage.
  - **Sub CPR missing**: Sent 2 days after prime payroll week is submitted when a registered sub has not submitted. Subject: "[Project Name] — subcontractor certified payroll overdue".
- Email templates: HTML templates in `src/server/email/templates/` using inline styles (no external CSS).

**Success Criteria:**
1. `STRIPE_PRICE_STARTER/PRO/ENTERPRISE` env vars read correctly; clicking Upgrade on any tier opens real Stripe Checkout session
2. Registration completes successfully without HCC membership number
3. ContactPage form submits to API; success state shows within 2s
4. LandingPage FAQ heading shows "Frequently Asked Questions"; no dev placeholder text remains in landing/marketing pages
5. TestimonialsPage shows no "(Demo account)" attribution text
6. Welcome email received within 30s of registration; deadline reminder cron fires without unhandled exceptions
...
### Phase 152: P0 Compliance Calculation Fixes

**Goal**: Fix three federal compliance calculation bugs that expose contractors to back-wage liability and DOL audit risk. These are mathematical correctness issues — wrong output on real payroll data — not UI issues. All three are P0 because a contractor relying on this software for federal certification is legally exposed if the calculations are wrong.

**Priority**: P0 — correctness/legal liability

**Depends on**: none

**Requirements**: COMP-FIX-01 through COMP-FIX-05

**Implementation details:**

**COMP-FIX-01 — Apprentice ratio: daily check, not weekly aggregate**
- Current state: `complianceService.ts:292` computes apprentice ratio using weekly aggregate hours (totalJourneyworkerHours / totalApprenticeHours across all days). Federal requirement (29 CFR Part 5.5(a)(4)) and most state requirements enforce the ratio per-trade per-day on jobsite, not as a weekly average. A contractor could have 6 apprentices Monday and 0 apprentices Friday, averaging fine but violating Monday.
- Fix: In `computeCompliance()`, change the apprentice ratio check from aggregate week to daily. Loop over `payrollEntries` grouped by `(trade, date)`. For each `(trade, date)` group where `journeyworkerHours > 0`: compute `apprenticeCount / journeyworkerCount` within that day-trade group; fire violation if ratio exceeds state/federal threshold.
- New violation type: `"apprentice-ratio-daily"` with payload `{ date, trade, apprenticeCount, journeyworkerCount, allowedRatio }`.
- This is a breaking change in violation types — update all tests that assert on `"apprentice-ratio"` to expect `"apprentice-ratio-daily"`.

**COMP-FIX-02 — DT hours hardcoded to 0 on import**
- Current state: `src/server/routes/import.ts:299` — when parsing imported timesheets (CSV/JSON), the `dtHours` (double-time hours) field is hardcoded to `0` regardless of what the import file contains. CA, AK, NV workers in imported timesheets always show 0 DT hours.
- Fix: Parse `dtHours` from import source. Alias table for CSV headers: `['dt_hours', 'double_time_hours', 'dt', 'doubletime', 'double_time']`. Validate: `dtHours >= 0`; DT hours cannot be set for non-CA/AK/NV states unless project explicitly allows DT.
- Update `tests/routes/import.test.ts` to add 3 new test cases: CA DT import, AK DT import, federal-project DT rejected.

**COMP-FIX-03 — Server-side gross wages deduction cap validation**
- Current state: The 29 CFR §3.5 deduction cap (30% of gross wages after taxes) is enforced in `complianceService.ts` but NOT validated on write in `payrollEntries` save route. A bad actor could save a payroll entry with deductions exceeding gross wages and the database would accept it.
- Fix: In `src/server/routes/payroll.ts` (the `POST /:projectId/weeks/:weekId/entries` handler), after computing `grossWages = (stHours + otHours + dtHours) × baseRate + allHours × fringeRate`, assert `totalDeductions <= grossWages`. Return 422 with body `{ error: "DEDUCTION_EXCEEDS_GROSS", message: "Total deductions ($X) cannot exceed gross wages ($Y)" }` if violated.
- No UI change needed — existing error toast infrastructure handles 422 responses.

**COMP-FIX-04 — weekEndingDate Saturday validation for federal projects**
- Current state: Federal WH-347 form requires work week ending on Saturday (29 CFR Part 3.4). Users can submit payroll weeks ending on any day — Sunday entries are valid in the DB but generate a technically non-compliant WH-347.
- Fix: In `POST /api/projects/:id/weeks` (create payroll week), for projects where `jurisdictionType === 'federal'`: validate that `new Date(weekEndingDate).getDay() === 6` (Saturday = 6). Return 422 `{ error: "WEEK_MUST_END_SATURDAY" }` if not. Show warning toast on the PayrollWeekDetailPage week-selector if week doesn't end Saturday.
- State projects are exempt — some states use Sunday as week-end date (NY, CA).

**COMP-FIX-05 — Guard baseRateSnapshot=0 entries from export**
- Current state: If a worker's wage determination was never fetched (rate = $0.00), payroll entries can be saved with `baseRateSnapshot = 0`. These entries export to WH-347 showing $0.00 wages — a federal form falsification issue.
- Fix: In `POST /api/projects/:id/weeks/:weekId/entries`, validate `baseRateSnapshot > 0`. Return 422 `{ error: "MISSING_WAGE_RATE", message: "Worker classification has no wage rate. Fetch wage determinations before entering payroll." }`.
- On PayrollWeekDetailPage, surface this as an inline banner above the add-entry form if any worker in the project has `baseRateSnapshot = 0`: "⚠ One or more workers have no wage rate set. Go to Workers to assign classifications and fetch rates."

**Success Criteria:**
1. `computeCompliance()` fires `apprentice-ratio-daily` for a test input with 6 apprentices on Monday and 0 on Friday — not passing the weekly aggregate
2. CSV import with `dt_hours` column populated correctly stores DT hours for CA projects; import of federal project with DT hours returns 422
3. `POST /api/payroll-entries` with `totalDeductions > grossWages` returns 422 `DEDUCTION_EXCEEDS_GROSS`
4. `POST /api/projects/:id/weeks` with non-Saturday date on federal project returns 422 `WEEK_MUST_END_SATURDAY`; state projects accept any day
5. `POST /api/payroll-entries` with `baseRateSnapshot = 0` returns 422 `MISSING_WAGE_RATE`
6. All 1,151+ existing tests still pass after fixes
...
### Phase 153: P0 Performance + DB Integrity

**Goal**: Fix the N+1 query bomb in `getBatchProjectCompliance` that fires O(projects × weeks × 5) DB queries per dashboard load, add missing migration journal entries that could cause silent schema drift, and add a unique constraint on `(projectId, payrollNumber)` to prevent duplicate certified payrolls.

**Priority**: P0 — performance degrades linearly with project count; DB integrity is a data correctness issue

**Depends on**: Phase 152 (compliance engine changes may affect compliance materialization)

**Requirements**: PERF-01, DB-01, DB-02

**Implementation details:**

**PERF-01 — Materialize compliance results**
- Current state: `src/server/services/projectService.ts` (or similar) calls `getBatchProjectCompliance(projectIds)` which, for each project, fetches all payroll weeks, then for each week fetches all entries, then runs `computeCompliance()` in memory. On a user with 20 projects × 10 weeks × 5 queries each = 1,000 DB queries per dashboard page load.
- Fix: Add a `compliance_cache` table:
  ```sql
  ...
  ```
  Write-through cache: whenever a payroll entry is saved/updated/deleted, invalidate the `compliance_cache` row for that `(projectId, weekId)`. `getBatchProjectCompliance` reads from cache first; only runs full compute if cache is missing or stale (older than 5 minutes).
- Dashboard load goes from O(N×W×5) to O(1) SELECT on compliance_cache with a WHERE project_id IN (...) query.

**DB-01 — Register missing migration journal entries**
- Current state: Migrations `0005_worker_address.sql`, `0006_gsa_rates.sql`, `0007_*` exist on disk but are missing from `src/server/db/migrations/meta/_journal.json`. Drizzle ORM silently skips files not in the journal. If a fresh database is created (new deployment, new developer environment), these migrations do not run — causing schema drift vs. what the code expects.
- Fix: Read the current journal; for each unregistered migration file in `src/server/db/migrations/`, compute its hash, add it to `_journal.json` entries array at the correct idx position. Run `npx drizzle-kit migrate` against a test DB to verify all migrations apply cleanly.
- Verification: `SELECT name FROM sqlite_master WHERE type='table'` shows all expected tables; `SELECT sql FROM sqlite_master WHERE name='workers'` includes `address` column.

**DB-02 — Unique constraint: (projectId, payrollNumber)**
- Current state: Two CPR submissions with the same `payrollNumber` for the same project can be created. This produces duplicate certified payrolls — federal form falsification. The compliance engine also double-counts hours if two weeks share a payroll number.
- Fix: Migration `ALTER TABLE payroll_weeks ADD CONSTRAINT uniq_project_payroll UNIQUE (project_id, payroll_number)`. The add-week API route should return 409 `DUPLICATE_PAYROLL_NUMBER` if violated. UI: on the "New Payroll Week" modal, auto-increment payroll number from the max existing number for that project (not always starting at 1).

**Success Criteria:**
1. Dashboard page with 20 projects fires ≤ 30 total DB queries (verified by query counter middleware or SQLite trace)
2. `compliance_cache` table exists; payroll entry save triggers cache invalidation for the correct week
3. Fresh DB init (delete `prevailing_wage.db`, restart server) applies all migrations including previously-unregistered ones; all expected columns present
4. Inserting two `payroll_weeks` rows with same `(project_id, payroll_number)` returns SQLITE_CONSTRAINT error; API returns 409
...
### Phase 154: DashboardPage UX Redesign

**Goal**: Transform the dashboard from a list of project cards with a static hero into a true command center where a contractor can assess their entire compliance posture in 10 seconds. Introduce a "Today" lane, a live compliance trend sparkline, and contextual quick actions that surface the next most important thing to do.

**Priority**: P1 — primary daily driver screen; competitors score higher here

**Depends on**: Phase 153 (needs fast unified dashboard endpoint)

**Requirements**: DASH-UX-01 through DASH-UX-06

**UX Path — Current State:**
1. User logs in → lands on DashboardPage
2. Hero section occupies ~40% of viewport with large headline and subtitle
3. Project cards grid below — each card shows project name, status badge, compliance badge, and a "View" button
4. No indication of what action to take next
5. "Due Soon" panel at bottom right — empty state was `return null` (invisible), now shows "All payroll is current" (fixed in prior session)
6. Stats row: Total Projects, Active Projects, On-Time Rate — static numbers, no trend
7. No way to search or filter projects inline (search is on a separate page)

**UX Path — After Phase 154:**
1. User logs in → hero is compact (48px tall, not 200px) — headline is one line: "Good morning, [FirstName]" with current date
2. **"Today" lane** (horizontal scroll row below hero): shows only items requiring action TODAY — overdue payroll weeks (amber chip), violations detected since last login (red chip), upcoming deadlines in 48h (gold chip). Each chip is a deep link to the specific project/week. If nothing due: shows green "All clear" chip.
3. **Stats row** (redesigned): 4 metric cards in a 2×2 grid on mobile, 4-wide on desktop — Total Projects, Compliance Rate (green/amber/red based on threshold), Active Weeks, Workers Covered. Each card shows a mini sparkline (7-day trend) from the compliance_cache materialized view.
4. **Project grid**: unchanged structure but adds inline search input above grid (type to filter by project name, contract number, or state). "Active / Archived" toggle becomes a segmented control instead of checkbox. Cards gain a "Last payroll: X days ago" subtitle line.
5. **Quick action strip**: below Today lane, 3 icon-buttons — "+ New Project", "Import CSV", "Download All CPRs" (batch ZIP). On mobile these are the bottom tab bar primary actions.
6. **Activity feed** (new, right-side panel on desktop, collapsed accordion on mobile): last 10 events across all projects — "Week 3 submitted for Caltrans I-5", "Maria Torres added to Sunrise Bridge", "Compliance violation cleared on San Jose BART". Click any event → deep-links to that item.

**Implementation details:**
- `DashboardPage.tsx`: replace hero `py-5` section with a `<div className="flex items-center justify-between py-3 px-4 border-b border-surface-card">` greeting bar
- Today lane: `<div className="flex gap-2 overflow-x-auto py-3 px-4 scrollbar-none">` — horizontal chip list; each chip is a `<Link>` with color variant
- Stats sparklines: use `recharts` (already in dependency tree) — `<Sparkline data={trend} width={60} height={20} />`; trend data comes from `dashData.complianceTrend` (already returned by unified /api/dashboard endpoint)
- Inline project search: `const [search, setSearch] = useState('')` — filter `projects` array on `project.name.toLowerCase().includes(search.toLowerCase())` — no API call (client-side filtering)
- Activity feed: add `activity` array to DashboardResponse type; populate in `src/server/routes/dashboard.ts` from `audit_log` table (last 10 records for the user's org, JOIN project name)
- Quick action strip: 3 `<Button>` variants — primary "New Project" (links to `/projects/new`), secondary "Import CSV" (opens WorkerCsvImportModal), secondary "Download All CPRs" (POST /api/export/batch)

**Success Criteria:**
1. Hero section height ≤ 56px on desktop; greeting shows first name from auth context
2. Today lane renders 0–N chips; overdue items appear in amber/red; "All clear" shows when nothing due
3. Stats sparklines render without error for 0-data (flat line) and multi-week data
4. Project search filters cards in real-time; no network request on keypress
5. Activity feed shows last 10 events; each event links to correct deep URL
6. Page passes Lighthouse performance score ≥ 90 with 20 projects loaded
...
### Phase 155: PayrollWeekDetailPage UX Overhaul

**Goal**: The payroll week detail page is the highest-frequency screen — contractors open it every payroll cycle. Currently it has no visual hierarchy between compliant and non-compliant sections, no indicator of which collapsible sections have problems, and the export button is always enabled even when the week has blocking violations. Fix all three and add a "print preview" mode.

**Priority**: P1 — highest-frequency page; highest impact on daily workflow

**Depends on**: Phase 152 (compliance fixes affect what violations appear here)

**Requirements**: PAY-UX-01 through PAY-UX-05

**UX Path — Current State:**
1. User navigates to a project → clicks a payroll week
2. Page shows: week selector at top, compliance summary card, then 3+ collapsible sections (Entries, Violations, Export)
3. Collapsible sections are all collapsed by default — user must expand each one to see status
4. Export/Download button is always active regardless of violation severity
5. On mobile: table-based entry list is unreadable (horizontal scroll, tiny tap targets)
6. No way to quickly see "is this week ready to submit?"

**UX Path — After Phase 155:**
1. User navigates to a project → clicks a payroll week
2. **Readiness banner** appears at top of page (below week selector): green "✓ Ready to certify" OR amber "⚠ X warnings — review before submitting" OR red "✗ X violations — must resolve before WH-347 can be downloaded". Banner is sticky on scroll.
3. **Collapsible section headers** gain colored left-border + icon indicator: green checkmark if section is clean, amber triangle if warnings, red X if blocking violations. User sees at a glance which section needs attention without expanding.
4. **Export button state machine**: 
   - No violations → gold "Download WH-347" button (full opacity)
   - Warnings only (non-blocking) → gold button with tooltip "X warnings present — review recommended before certifying"
   - Blocking violations → button shows red outline, disabled, tooltip "Resolve X violations before downloading"
   - Override: "Download Anyway" secondary button appears below for blocking violations (requires typing "OVERRIDE" in a confirm input — creates audit log entry)
5. **Mobile entry view**: replaces horizontal-scroll table with `MobilePayrollEntryCard` accordion (built in prior session). ST/OT inputs are large tap targets. Day pills (M-T-W-Th-F-Sa-Su) highlight days with hours entered. Gross estimate updates on blur.
6. **Print preview mode**: new `?print=1` query param that hides nav/sidebar, shows only the payroll week data in WH-347 column layout. Accessible via "Preview" button next to export button.

**Implementation details:**
- Readiness banner: computed from `violations` array — `const severity = violations.some(v => v.severity === 'critical') ? 'error' : violations.length > 0 ? 'warning' : 'success'`
- Section header indicators: pass `status: 'ok' | 'warning' | 'error'` prop to each `<Collapsible>` component — renders left border `border-l-4 border-green-500` / `border-amber-500` / `border-red-500`
- Export button state: add `exportDisabled` derived state — `const exportDisabled = violations.some(v => BLOCKING_VIOLATION_TYPES.includes(v.type))`. BLOCKING_VIOLATION_TYPES = `['under-wage', 'missing-fringe', 'deduction-cap-exceeded']`
- Override workflow: `showOverrideModal` state; modal has `<input type="text" placeholder='Type "OVERRIDE" to confirm'>` — POST /api/audit-log `{ action: 'export-override', weekId, userId }` before unblocking download
- Print preview: `if (searchParams.get('print') === '1') return <PrintLayout week={weekData} />`

**Success Criteria:**
1. Readiness banner changes color/text correctly for 3 violation states; sticky on scroll
2. Section headers show colored indicator matching their content state
3. Export button disabled when blocking violations present; override modal fires audit log entry
4. MobilePayrollEntryCard renders on viewport < 640px; table renders on ≥ 640px
5. `?print=1` URL shows print layout; nav and sidebar hidden; browser print dialog produces clean output
...
### Phase 156: WorkersPage UX — Slide-Over Edit Panel + CSV Import Polish

**Goal**: Adding and editing workers is currently a page-navigation workflow (navigate away, fill form, navigate back). Replace with an inline slide-over panel for a zero-navigation editing experience. Polish the CSV import modal with better error UX and column-mapping assistance.

**Priority**: P1 — workers are entered every new project

**Depends on**: none (CSV import built in prior session; this polishes it)

**Requirements**: WRK-UX-01 through WRK-UX-04

**UX Path — Current State:**
1. User on WorkersPage sees worker table/cards
2. Clicks "Add Worker" → navigates to `/projects/:id/workers/new` full page form
3. Fills form, submits → navigates back to WorkersPage
4. To edit a worker → clicks worker name or edit icon → navigates to `/projects/:id/workers/:workerId/edit` → fills form → navigates back
5. Total round-trip: 2 page loads per add/edit operation
6. CSV import: upload CSV → mapping step shows raw column names, no suggestion → preview shows truncated table → result shows only count

**UX Path — After Phase 156:**
1. User clicks "Add Worker" → **slide-over panel** animates in from the right (320px wide on desktop, full-width on mobile). Background is dimmed but still scrollable. Worker list remains visible behind the overlay.
2. Slide-over contains the full add-worker form: Name, Classification, Trade, Labor Type (journeyworker / apprentice / foreman), Base Rate, Fringe Rate, SSN (masked), Address (optional).
3. User submits → slide-over closes, worker appears in the list immediately (optimistic UI via TanStack Query mutation + `queryClient.invalidateQueries(['workers', projectId])`). Success toast: "Maria Torres added."
4. To edit: click worker row → same slide-over opens pre-populated with that worker's data. Edit fields, submit → "Changes saved" toast.
5. Delete: red "Remove" button at bottom of slide-over. Requires clicking "Remove" then confirming in an inline confirm state within the slide-over (not a separate modal).
6. **CSV Import polish**:
   - Column mapping step now shows auto-detected suggestions highlighted in gold: "We detected 'worker_name' → Name ✓, 'trade_class' → Classification ✓, 'ssn' → SSN ✓". Unmatched columns shown in gray with a dropdown to manually map.
   - Preview table is full-width, shows first 10 rows, columns are color-coded: green = mapped, gray = unmapped (will be skipped), red = validation error.
   - Result step shows 3 buckets with counts and lists: Created (N), Skipped duplicates (N), Errors (N with expandable per-row error).
   - "Download error report" button on result step generates a CSV of the failed rows with error reason column.

**Implementation details:**
- Slide-over: `<div className="fixed inset-y-0 right-0 w-80 sm:w-96 bg-surface-card shadow-2xl transform transition-transform duration-200 z-50">` with `translate-x-full` when closed, `translate-x-0` when open
- Slide-over header: `<div className="flex items-center justify-between p-4 border-b border-surface-card">` with title and X close button
- Optimistic UI: `useMutation` with `onMutate` callback that manually adds the new worker to the `['workers', projectId]` query cache before the API call resolves
- CSV import `errorReport`: `const csvContent = errors.map(e => `${e.row},${e.error}`).join('\n')` → `Blob` → `URL.createObjectURL()` → anchor click

**Success Criteria:**
1. Slide-over opens/closes with CSS transition; background dims but does not prevent scroll
2. Worker added via slide-over appears in list immediately (optimistic); success toast shown
3. Worker edit pre-populates all fields; save updates list without page reload
4. CSV import column mapping step shows auto-detected columns highlighted; undetected columns show dropdown
5. CSV import result shows 3 buckets; "Download error report" button produces valid CSV
...
### Phase 157: FieldClockPage UX — Crew Status Banner + Worker Grouping

**Goal**: The field clock page is used by foremen on mobile during construction. Currently it has no way to see the crew's overall status (who's clocked in vs. out), the back button navigates to a broken route, and entries are listed in the order they were created rather than grouped by worker. Fix all three.

**Priority**: P1 — field UX directly affects whether foremen adopt the app

**Depends on**: none

**Requirements**: FIELD-UX-01 through FIELD-UX-04

**UX Path — Current State:**
1. Foreman opens FieldClockPage on phone
2. Sees a list of clock-in entries in chronological order — no way to see which workers are currently clocked in vs. out
3. Back button in top-left → navigates to `/` (root) instead of `/projects/:id` — incorrect
4. GPS clock-in fires immediately on button tap — no confirmation, no indication of GPS accuracy
5. No way to quickly "clock out all crew" at end of shift

**UX Path — After Phase 157:**
1. **Crew status banner** at top: "4 of 7 workers clocked in ●●●●○○○" (dot indicators). Green dot = clocked in, gray dot = not clocked in today. Tapping the banner expands to show the full crew list with status per worker.
2. **Worker grouping**: entries are grouped by worker (not chronological). Each worker has an accordion card showing: worker name, current status (In / Out), hours so far today, and a clock-in / clock-out button. Chronological order shows WITHIN each worker's card (their in/out events for the day).
3. **Back button fix**: replace hardcoded `/` with `useNavigate(-1)` or derive from `location.state.from` passed when navigating to the field clock page.
4. **GPS accuracy indicator**: after GPS lookup, show accuracy ring around the "Clock In" button: `< 10m = green (good)`, `10-50m = amber (fair)`, `> 50m = red (poor — tap to retry GPS)`. GPS accuracy is included in `GeolocationPosition.coords.accuracy`.
5. **Clock out all crew** button: appears at bottom when ≥ 2 workers are clocked in. Tap → "Clock out all 4 workers?" confirmation → batch POST `/api/field/clock-out-all` → "4 workers clocked out" toast.

**Implementation details:**
- `GpsClockIn.tsx`: add `accuracy` to GPS callback state; render accuracy ring via inline SVG circle with stroke-dasharray
- `FieldClockPage.tsx`: group entries by `workerId`; sort workers by status (clocked in first, then out)
- Back button: `<button onClick={() => navigate(-1)}>` (replaces `<Link to="/">`); or if history is empty, fall back to `/projects/${projectId}`
- Batch clock-out: `POST /api/projects/:projectId/field/clock-out-all` — server queries all open clock-in events for the project+date and closes them

**Success Criteria:**
1. Crew status banner shows correct count of clocked-in workers; dot indicators match worker statuses
2. Entries grouped by worker with accordion; chronological within each worker
3. Back button returns to previous page (project detail), not root
4. GPS accuracy ring shows correct color based on `coords.accuracy` value
5. "Clock out all" button appears when ≥ 2 workers in; batch API call closes all open entries
...
### Phase 158: GlobalReportsPage UX — Column Sort, Date Filter, Token Fix

**Goal**: GlobalReportsPage is the compliance officer's primary view for audits and portfolio oversight. Currently it has unsortable columns (clicking headers does nothing), no date range filter (shows all time by default), and uses a `brand-navy` CSS token that doesn't exist in the theme (renders as transparent, causing invisible text in some cells). Fix all three.

**Priority**: P1 — compliance officers evaluating the product specifically test this page

**Depends on**: Phase 153 (performance — GlobalReports may execute similar N+1 queries)

**Requirements**: RPT-UX-01 through RPT-UX-04

**UX Path — Current State:**
1. Compliance officer opens `/reports` (global reports)
2. Sees a large data table with columns: Project, Week, Workers, OT Hours, Fringe Total, Compliance Status
3. Clicks a column header expecting to sort — nothing happens
4. Date range is "all time" with no way to filter — performance degrades with large datasets
5. Some cells have invisible text (brand-navy token missing from theme)
6. No way to export the report to CSV

**UX Path — After Phase 158:**
1. **Column sort**: clicking any column header cycles sort: asc → desc → none. Active sort column shows ▲/▼ icon. Sort state is URL-persisted (`?sort=weekEndingDate&dir=desc`) so links can be shared.
2. **Date range filter**: new filter bar above the table — "Period" select: Last 7 days / Last 30 days / This quarter / This year / Custom. Custom shows two date inputs. Selection fires a new API query with `startDate` + `endDate` params. Default: "Last 30 days" (not all time).
3. **Token fix**: grep for `brand-navy` across all client files; replace with `text-nav-dark` or `bg-nav-dark` (the correct token names from `@theme` in `index.css`). All cells render correctly.
4. **CSV export**: "Export CSV" button in top-right of page. Client-side: takes the currently-rendered/filtered table rows and serializes to CSV via `Array.map(row => [row.project, row.week, ...].join(','))` + Blob download. No server call needed.
5. **Compliance status column**: replace plain text ("Compliant" / "1 Violation") with `<Badge variant="compliant">` / `<Badge variant="violation" count={N}>` — visual scan at a glance.

**Implementation details:**
- Sort state: `const [sortCol, setSortCol] = useState<string | null>(null); const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc')`. Sort applied client-side on rendered data (no new API call for basic string/number sorts).
- URL persistence: `useSearchParams` — on sort change: `setSearchParams({ sort: col, dir: direction })`
- Date range API: `GET /api/reports/global?startDate=2026-01-01&endDate=2026-03-31` — server already accepts these params (or add if not present)
- Token audit: `grep -r "brand-navy" src/client` — list all occurrences → batch replace

**Success Criteria:**
1. Clicking column headers cycles sort asc/desc/none; sort state persists in URL query params
2. Date range filter "Last 30 days" is default; selecting "Custom" shows date inputs; changing period fires API query
3. No `brand-navy` token usage remains in `src/client/`; all text cells render visible
4. "Export CSV" downloads a valid CSV file matching the current filtered/sorted view
5. Compliance status column uses Badge component with correct variant and violation count
...
### Phase 159: Auth UX — Google OAuth + Password Strength + HCC# Optional

**Goal**: Registration friction is the #1 drop-off point for SaaS products. Add Google OAuth as a zero-friction registration path, add a password strength meter so users don't fail server-side validation without guidance, and confirm that HCC# is optional (fixed in Phase 151 — this phase adds the UX polish for that change).

**Priority**: P1 — directly impacts conversion rate

**Depends on**: Phase 151 (HCC# optional server-side fix must be deployed first)

**Requirements**: AUTH-UX-01 through AUTH-UX-04

**UX Path — Current State:**
1. User lands on `/register`
2. Sees: HCC Member # (required), Full Name, Company, Email, Password, Confirm Password
3. Creates weak password → form submits → server returns 422 "Password must be 8+ chars with uppercase" → generic red toast shows; no inline guidance
4. No OAuth option

**UX Path — After Phase 159:**
1. User lands on `/register`
2. At top: "Continue with Google" button (full-width, white with Google logo). Below a horizontal "or" divider. Then the manual form.
3. HCC Member # field labeled "HCC Member # (optional)" with helper text under field.
4. Password field has **strength meter** below it: 5-segment bar that fills left-to-right as complexity increases. Segment colors: gray → red (1) → orange (2) → amber (3) → yellow-green (4) → green (5). Label: "Weak / Fair / Good / Strong / Very Strong". Requirements checklist appears when password field is focused: "✓ 8+ characters", "✓ Uppercase letter", "✓ Number or symbol" — each turns green as requirement is met.
5. "Continue with Google" → server redirects to Google OAuth consent screen → callback at `/auth/google/callback` → server creates or finds user → sets session cookie → redirects to `/dashboard?welcome=1`. If user already exists: logs them in. No password needed.
6. `/dashboard?welcome=1`: shows a welcome modal (one-time, dismissed on close, stored in localStorage).

**Implementation details:**
- Google OAuth: `passport.js` with `passport-google-oauth20` strategy. Env vars: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`. Callback URL: `${process.env.APP_URL}/auth/google/callback`.
- Password strength: pure client-side scoring — count: lowercase, uppercase, digit, symbol, length. Score 0-5 based on criteria met. Segment bar: `<div className="flex gap-1 mt-1">` with 5 `<div className="h-1 flex-1 rounded">` each colored based on score.
- Requirements checklist: `useMemo` derived from `password` field value — re-renders on each keystroke.

**Success Criteria:**
1. "Continue with Google" button renders on RegisterForm and LoginForm; clicking initiates OAuth flow (redirects to Google); callback creates session and redirects to dashboard
2. Password strength bar updates on every keystroke; requirements checklist shows correct green/gray per criterion
3. HCC# field shows "(optional)" label and helper text; registration succeeds without it
4. Welcome modal shows on `/dashboard?welcome=1`; dismissed on click; does not show again on next page load
...
### Phase 160: OnboardingChecklist + DueSoonPanel Polish

**Goal**: The onboarding checklist is the first thing new users see on the dashboard. Currently it counts optional steps toward the progress percentage (misleading), doesn't deep-link to each step's destination, and has no "dismiss" capability. The DueSoonPanel (recently fixed to show "All payroll is current" state) needs deadline urgency indicators and a "snooze" capability.

**Priority**: P2 — improves new user activation; current state is functional but rough

**Depends on**: Phase 154 (DashboardPage redesign may relocate these panels)

**Requirements**: ONBOARD-UX-01, ONBOARD-UX-02, DUE-UX-01, DUE-UX-02

**UX Path — OnboardingChecklist current state:**
1. New user registers → dashboard shows checklist: 8 items, progress bar at 12% (1/8 complete)
2. Items include optional steps (e.g., "Link HCC membership", "Set up MFA") counted toward the 87% they haven't done
3. Clicking a checklist item does nothing — no deep link
4. No way to dismiss the checklist when "done enough"

**UX Path — OnboardingChecklist after Phase 160:**
1. New user registers → dashboard shows checklist with **required steps only** counted in progress: "Create your first project", "Add a worker", "Enter first payroll week", "Download your first WH-347". Progress = 0/4 (0%).
2. **Optional steps** shown in a separate expandable section "Optional setup" with a "(optional)" label — not counted in progress %.
3. Clicking any step → navigates to the correct deep-link destination (e.g., "Create first project" → `/projects/new`; "Add a worker" → `/projects/{mostRecentProjectId}/workers/new`).
4. Checklist auto-dismisses when all 4 required steps are complete. User sees a "🎉 Setup complete! You're ready to certify payroll." toast.
5. "Dismiss for now" link at bottom right of checklist — hides the checklist for 7 days (stored in `user_preferences` table).

**UX Path — DueSoonPanel current state:**
1. Panel shows items due in next 7 days
2. Items listed as plain text rows: "Project Name — Week 3 due Fri"
3. "All caught up" state: green card (fixed in prior session)
4. No urgency distinction (due tomorrow vs. due in 6 days look identical)
5. No snooze

**UX Path — DueSoonPanel after Phase 160:**
1. Items have urgency coloring: **red** = due in 0-1 days (TODAY / TOMORROW), **amber** = due in 2-3 days, **gray** = due in 4-7 days.
2. Each item shows a relative time label: "Due in 3 hours", "Due tomorrow", "Due in 5 days".
3. "Snooze 24h" icon button (bell-slash icon) on each item — removes item from panel for 24 hours (stored in `user_preferences`). Snoozed item shows "(snoozed)" in gray if user toggles "Show snoozed".
4. "View all" link at bottom opens a `/deadlines` full-page view with all upcoming + past deadlines.

**Success Criteria:**
1. OnboardingChecklist progress % counts only 4 required steps; optional steps in collapsed section
2. Clicking any checklist item navigates to correct destination
3. Checklist auto-dismisses at 4/4; success toast shown
4. DueSoonPanel items color-coded by urgency; relative time label visible
5. Snooze button removes item for 24h; persists across page reload
...
### Phase 161: QuickBooks 2-Way Write-Back

**Goal**: Complete the QuickBooks integration by adding write-back: when a payroll week is certified in the app, automatically post the payroll expense entries back to QuickBooks Online as Journal Entries. Currently the integration is read-only (QBO → PW). Write-back closes the gap vs. LCPtracker and Procore which both advertise bi-directional QBO sync.

**Priority**: P1 — top competitive gap; QBO sync is the #1 requested feature in pilot feedback

**Depends on**: Phase 126 (QBO OAuth/read sync built in v9.0)

**Requirements**: QBO-WB-01 through QBO-WB-04

**UX Path:**
1. User certifies payroll week (clicks "Download WH-347" — or "Submit" in future submission tracking)
2. If QBO is connected (user has valid QBO token): after PDF download, a secondary toast appears: "Payroll exported to QuickBooks — Journal Entry #1234 created." with a "View in QBO →" link.
3. If QBO is not connected: no toast (non-blocking). User can navigate to Settings → Integrations → "Connect QuickBooks" to enable.
4. In Settings → Integrations: new "Write-back" toggle. When enabled, certified payrolls auto-post. When disabled, user can manually trigger sync per week from PayrollWeekDetailPage ("Sync to QBO" button).

**Implementation details:**
- New service: `src/server/services/qboWriteBack.ts` — `postPayrollJournalEntry(weekId: string, qboTokens: QBOTokens)` — aggregates week entries into debit lines (per worker, per trade) against QBO expense accounts configured in Settings. Posts to QBO Journal Entries API.
- Trigger: in `POST /api/projects/:id/weeks/:weekId/certify` (or on WH-347 download) — fire `qboWriteBack.postPayrollJournalEntry()` if `org.qboConnected && org.qboWriteBackEnabled`.
- QBO account mapping: `qbo_account_mapping` table — `{ orgId, tradeCode, qboAccountId }`. Settings page lets users map trades to QBO expense accounts.
- Idempotency: check `payroll_weeks.qbo_journal_entry_id` before posting — skip if already synced.

**Success Criteria:**
1. Certifying a payroll week (QBO connected, write-back enabled) creates a Journal Entry in QBO sandbox
2. Journal Entry lines match worker entries (debit = gross wages per trade, credit = payroll liability)
3. `payroll_weeks.qbo_journal_entry_id` populated after sync; second certification attempt does not create duplicate
4. Write-back toggle in Settings persists; disabling stops auto-post
5. "View in QBO →" link opens correct QBO Journal Entry in browser
...
### Phase 162: Agency Portal / Reviewer Login

**Goal**: Let awarding agencies, labor compliance officers, and prime contractors invite read-only reviewers into the system to verify certified payrolls without giving them editing access. This is a top request from government agency pilot users and a feature that B2Gnow has (their primary differentiator).

**Priority**: P1 — without this, agencies can't adopt HCC PW as their official portal; they use B2Gnow instead

**Depends on**: Phase 35 (RBAC system)

**Requirements**: AGENCY-01 through AGENCY-05

**UX Path — Agency User:**
1. Prime contractor or admin invites an agency reviewer from TeamSettings → "Invite Reviewer" tab → enters email + name + role = "Agency Reviewer"
2. Reviewer receives email: "You've been invited to review certified payrolls for [Project Name] on HCC Prevailing Wage"
3. Reviewer accepts invite → lands on registration with role pre-set = `agency_reviewer` → no HCC# required
4. Reviewer logs in → sees a **read-only portal view**: project list (only projects they've been invited to). Per project: list of payroll weeks with compliance status. Click week → full read-only PayrollWeekDetailPage (no edit buttons, no add-entry, no archive).
5. Reviewer can **comment on a week**: "Please resubmit — apprentice ratio appears incorrect for week 3". Comment appears in prime's notification feed.
6. Reviewer can **stamp a week as "Reviewed"**: fires a server event that updates `payroll_weeks.review_status = 'reviewed'` and notifies prime.

**Implementation details:**
- New role: `agency_reviewer` — added to `user_roles` enum. Role grants: `project:read`, `payroll:read`, `comments:write`, `reviews:stamp`. No `payroll:write`, `project:create`, `export:download`.
- Route guard: `ProtectedRoute` reads role from session; renders `AgencyPortalLayout` (no sidebar nav, simplified header, "Read Only" badge) when `role === 'agency_reviewer'`.
- New table: `reviewer_project_access` — `{ reviewerId, projectId, invitedAt, invitedBy }` — reviewers only see projects in this table.
- Comment thread: `payroll_week_comments` table — `{ weekId, authorId, body, createdAt }`. Render on PayrollWeekDetailPage in an accordion "Reviewer Comments" at bottom.
- Review stamp: `PATCH /api/projects/:id/weeks/:weekId` with `{ review_status: 'reviewed' }` — only `agency_reviewer` role can set this.

**Success Criteria:**
1. Admin can invite reviewer via TeamSettings; reviewer receives email and completes registration
2. Reviewer login shows read-only portal; no edit/add buttons visible; "Read Only" badge in header
3. Reviewer only sees projects they've been explicitly invited to; accessing other project IDs returns 403
4. Reviewer can add comment on a week; prime sees comment in notification feed
5. Reviewer can stamp week "Reviewed"; status updates in prime's payroll week list
...
### Phase 163: Help Center + In-App Guidance

**Goal**: New users spend the first 2 days confused about prevailing wage terminology (Davis-Bacon, fringe, CWHSSA, WH-347) and app workflow. Add an in-app help center (searchable FAQ + glossary), contextual "?" tooltips on every confusing term, and a guided tour for first-time users. This directly reduces support ticket volume and improves trial-to-paid conversion.

**Priority**: P1 — competitive gap; LCPtracker has a help center; we have none

**Depends on**: none

**Requirements**: HELP-01 through HELP-05

**UX Path:**
1. Any authenticated user sees a "?" help button in the bottom-right corner (fixed position, above bottom tab bar on mobile). Clicking opens a slide-out Help Center panel.
2. Help Center panel has: Search bar at top, then tabbed sections: "Getting Started", "Payroll", "Compliance", "Forms", "Glossary".
3. **Glossary tab**: searchable list of terms — Davis-Bacon Act, Prevailing Wage, WH-347, Certified Payroll, CWHSSA, Journeyworker, Apprentice, Fringe Benefits, Deduction Cap, Wage Determination. Each term has a 2-3 sentence definition. Terms in the app UI (`<TermTooltip>` component) link to the glossary.
4. **Guided first-time tour**: `driver.js` (lightweight tour library, ~15kb). Fires on first login (`user.tourCompleted === false`). 5 steps: Dashboard → Create Project button → Workers page → Payroll entry → Download WH-347. Each step has a highlighted element, tooltip title, and 1-sentence description. "Skip tour" option.
5. **Contextual "?" icons** on: PayrollWeekDetailPage "Compliance Summary" header, WorkersPage "Labor Type" column header, every state form name in the export dropdown. Clicking shows a Tooltip with a 2-sentence explanation.

**Implementation details:**
- Help Center panel: `src/client/components/help/HelpCenterPanel.tsx` — fixed position div, z-index 100, slide-in from right on click of help FAB
- Tour: `npm install driver.js` — configure steps in `src/client/lib/tour.ts`; read/write `user_preferences.tour_completed` via `PATCH /api/users/me/preferences`
- Glossary data: `src/client/lib/glossary.ts` — static array of `{ term, definition, related[] }` — no API needed
- TermTooltip: already exists in `src/client/components/ui/TermTooltip.tsx` — wire it to glossary data

**Success Criteria:**
1. Help FAB visible on every authenticated page; clicking opens Help Center panel
2. Glossary has ≥ 15 terms with definitions; search filters correctly
3. Tour fires on first login; 5 steps highlight correct elements; "Skip" dismisses and sets `tour_completed = true`
4. TermTooltip on "Fringe Benefits", "Apprentice Ratio", and "WH-347" shows correct glossary definition on hover
5. No console errors on help panel open/close
...
### Phase 164: Supervisor Time-Punch Approval Workflow

**Goal**: Add a two-step clock-in/clock-out approval flow where the foreman on-site clocks in workers and the project supervisor (office role) reviews and approves the day's time before it's locked into payroll. This mirrors how paper time cards work in construction and is required by several GC compliance programs (Caltrans, NYC DDC).

**Priority**: P1 — Caltrans and NYC DDC pilot users require supervisor sign-off before CPR

**Depends on**: Phase 157 (FieldClockPage improvements), Phase 35 (RBAC)

**Requirements**: CLOCK-APPROVE-01 through CLOCK-APPROVE-04

**UX Path:**
1. Foreman uses FieldClockPage to clock workers in/out throughout the day.
2. At end of shift: foreman taps "Submit for approval" (new button at bottom of FieldClockPage). All that day's clock entries transition from `status: 'pending'` to `status: 'submitted'`. Foreman cannot edit after submitting.
3. Supervisor receives push notification (or email): "[Foreman Name] submitted time for [Project Name] — [Date]. 6 workers, 48 total hours. Tap to review."
4. Supervisor opens a new **Time Approval page** (`/projects/:id/time-approval`): shows a day-by-day accordion. Each day shows a table of workers × hours (ST/OT/DT). Supervisor can: **Approve** (locks entries — moves to `status: 'approved'`), **Reject with note** (sends back to foreman with rejection reason), or **Edit directly** (supervisor role can correct hours before approving).
5. Approved entries are the only entries that appear in the Payroll Entry form when creating a payroll week — foreman's unapproved time is blocked from becoming CPR.

**Implementation details:**
- New `clock_entry_status` column on `field_clock_entries`: `'pending' | 'submitted' | 'approved' | 'rejected'`
- `POST /api/projects/:id/field/submit-day` — transitions all of user's `pending` entries for the given date to `submitted`
- `POST /api/projects/:id/time-approval/:date/approve` — transitions `submitted` → `approved`; only `supervisor` or `admin` role
- `POST /api/projects/:id/time-approval/:date/reject` — requires `rejectionReason` body; transitions `submitted` → `rejected`; notifies foreman
- PayrollEntryForm: when populating hours from clock records, filter `WHERE status = 'approved'`

**Success Criteria:**
1. Foreman can submit day's time; entries transition to `submitted`; foreman edit buttons disabled after submit
2. Supervisor receives notification (email); Time Approval page shows correct entries grouped by date
3. Approving a day locks entries to `approved`; rejecting sends reason to foreman
4. PayrollEntryForm pre-fill only uses `approved` clock entries, not `pending` or `submitted`
5. Foreman who submitted sees "Awaiting approval" badge on each submitted day in FieldClockPage
...
### Phase 165: Sub-of-Sub (Tier 3) Tracking + DBE Participation Report

**Goal**: Federal and state contracts commonly have 3 tiers: Prime → Sub → Sub-of-Sub. Currently the system only tracks 2 tiers (Prime and Sub). Add tier-3 tracking so that all certified payrolls chain correctly. Also build the DBE Participation Report (exportable PDF/CSV) that shows actual hours and wages paid to DBE/MBE/WBE firms, required by many federal grants (BUILD, RAISE, IIJA).

**Priority**: P1 — federal highway and transit projects require tier-3 CPR chains

**Depends on**: Phase 40 (sub CPR upload portal)

**Requirements**: SUB-TIER3-01, SUB-TIER3-02, DBE-RPT-01, DBE-RPT-02

**UX Path — Tier 3:**
1. A Sub contractor in the system can navigate to their project → "Subcontractors" tab → "Add Sub-of-Sub".
2. The same CPR upload portal used for Sub CPRs now accepts tier-3 submissions. The submission chain shows as: Prime → [Sub] → [Sub-of-Sub] with indentation in the compliance tree.
3. Prime's compliance dashboard shows a tier-3 indicator: "Subcontractor chain: Prime (you) → Martinez Electric → B&B Wiring. All CPRs submitted."
4. Missing tier-3 CPRs generate the same `sub-cpr-missing` violation as tier-2 missing CPRs.

**UX Path — DBE Participation Report:**
1. From GlobalReportsPage → new tab "DBE Participation".
2. Filter by: date range, project, DBE tier (1st-tier only / all tiers). 
3. Table shows: Firm Name, DBE Certification Type (DBE / MBE / WBE / SDVOSB), Contract Value, Hours Worked, Wages Paid, % of Project Total.
4. "Export PDF" generates a branded report matching the format required by DOT's D/M/WBE utilization report.
5. "Export CSV" for agency submission portals.

**Implementation details:**
- `subcontractors` table: add `parent_sub_id TEXT` (NULL for tier-2 subs, set for tier-3). Recursive CTE query to build the full sub chain.
- DBE report: JOIN `subcontractors` on `dbe_certified = 1` with `payroll_weeks` entries — aggregate `totalHours`, `grossWages` per firm.
- DBE PDF: new `src/server/services/dbeReportGenerator.ts` using pdf-lib.

**Success Criteria:**
1. Sub can add a tier-3 sub; tier-3 appears in the compliance chain with correct indentation
2. Missing tier-3 CPR fires `sub-cpr-missing` violation on prime's compliance summary
3. DBE Participation Report shows correct aggregated hours/wages per DBE firm
4. DBE PDF export matches expected DOT format (firm name, cert type, contract %, hours, wages)
...
**Goal**: Add Service Contract Act (SCA) compliance mode for federal service contracts (janitorial, security, maintenance). SCA uses DOL Wage Determination by state/county/occupation — different from Davis-Bacon which is by construction trade. This unlocks a new market segment (federal facility management contractors) currently using paper/Excel.

**Priority**: P1 — $2B+ federal service contract market; direct competitors (Wage Indicator, LaborSoft) serve this segment

**Depends on**: Phase 74 (jurisdictionType column on wage_determinations — already added)

**Requirements**: SCA-01 through SCA-05

**UX Path:**
1. Project creation wizard: new step "Project Type" — "Construction (Davis-Bacon)" vs. "Service (SCA)". Selecting SCA changes the wage determination fetch flow.
2. SCA projects fetch WDs from DOL's SCA WD database (different API endpoint than Davis-Bacon WDOL). Classification lookup shows SCA occupation codes (e.g., "07010 — Computer Operator I") instead of construction trades.
3. Payroll entry for SCA: no OT premium different from FLSA standard (SCA uses FLSA OT rules, not CWHSSA). No daily OT thresholds. Fringe is as-stated in WD.
4. SCA CPR form: WH-347 is NOT the correct form for SCA. SCA uses no standard federal form — agencies typically accept a spreadsheet. Generate an Excel/CSV report in the format each agency prefers, or use a configurable template.
5. Compliance engine for SCA: check wages against SCA WD rate by occupation; check health + welfare fringe against WD health rate; no apprentice ratio enforcement (SCA has none).

**Implementation details:**
- `projects` table: add `project_type TEXT NOT NULL DEFAULT 'davis_bacon'` — possible values: `'davis_bacon' | 'sca' | 'state_pw'`
- SCA WD fetch: `GET https://api.sam.gov/wage-determinations` (or DOL API) with county/state/occupation filter
- SCA-specific compliance rules in `complianceService.ts`: dispatch on `project.projectType === 'sca'`
- SCA export: generate a structured Excel via `exceljs` or CSV; not a WH-347

**Success Criteria:**
1. Project creation wizard shows "Service (SCA)" option; selecting it changes WD fetch source
2. SCA WD fetched and cached for at least 3 occupation codes in a test state
3. SCA payroll entries saved without apprentice ratio or CWHSSA rules firing
4. SCA export generates a valid CSV/Excel with correct column headers and worker data
5. `jurisdictionType = 'sca'` correctly identified on wage_determination records
...
### Phase 167: WA F700 Form Fix + Offline Conflict Resolution

**Goal**: The WA F700 form is completely non-functional — all coordinate values are `{ x: 0, y: 0 }`, so every text element is drawn in the top-left corner, overlapping and unreadable. Fix it to match the actual WA L&I F700-074-000 form layout. Also implement offline conflict resolution for the Progressive Web App — currently, edits made offline silently overwrite online changes when syncing.

**Priority**: P1 — WA is a high-construction-activity state; F700 is required for all WA public works. Offline conflict is a data integrity risk.

**Depends on**: none

**Requirements**: WA-F700-01, OFFLINE-01, OFFLINE-02

**WA F700 Fix:**
- Download the actual WA F700-074-000 form from L&I website.
- Open in Adobe Acrobat or pdf-lib inspection to measure coordinate grid.
- Map field positions: project name (x=72, y=720), contract number (x=310, y=720), week ending (x=480, y=720), worker name (x=72, y=680 per row), SS# (x=200, y=680), trade (x=260, y=680), hours (x=350–420, y=680), wages (x=430, y=680), deductions (x=490, y=680), net (x=540, y=680) — exact values require measuring the actual form PDF.
- Update all values in `src/server/services/waF700Generator.ts` column constant map.
- Add test: `tests/services/waF700Generator.test.ts` — WA-F700-01 through WA-F700-05 — PDF generated, magic bytes ≥ %PDF, worker name appears, does not throw on missing optional fields.

**Offline Conflict Resolution:**
- Current behavior: offline edit creates a local queue entry. On reconnect, queue flushes with PUT requests. If the server has a newer version (another user edited), the local offline version overwrites it.
- Fix: Implement a "last-write-wins with conflict detection" strategy. Each record has a `serverVersion` integer (already in schema as `updatedAt` timestamp). On PUT, send `If-Match: {clientVersion}` header. Server returns 409 Conflict with `{ serverVersion, serverData }` if client version is stale. Client shows: "This record was updated by another user while you were offline. Choose which version to keep: [Yours] [Theirs]". User picks one → final PUT accepted.

**Success Criteria:**
1. WA F700 PDF generated without errors; all coordinate values non-zero; worker name appears in correct position on the form
2. WA-F700-01 through WA-F700-05 tests pass
3. Offline edit queue flushes on reconnect
4. If server version is newer than client version, 409 returned; conflict UI shows both versions; user choice resolves to final write
...
### Phase 168: Security Hardening — Session Cookies + SSN Audit

**Goal**: Fix two security issues identified in the audit: (1) session cookie `sameSite` attribute is not set to `strict` (currently `lax`), allowing CSRF on some cross-site requests; (2) confirm SSN is never returned in any API response or logged — conduct a full audit of all API routes.

**Priority**: P1 — security posture required for government agency adoption
...
**SEC-01 — sameSite: strict session cookie**
- File: `src/server/index.ts` (or wherever `express-session` is configured). Change `cookie: { sameSite: 'lax' }` to `cookie: { sameSite: 'strict' }`. This prevents the session cookie from being sent on cross-site requests, eliminating CSRF risk.
- Verify: `Set-Cookie` response header shows `SameSite=Strict`.
- Caveat: `strict` breaks OAuth redirects (Google OAuth sends the callback as a cross-site navigation). Workaround: use a separate short-lived OAuth state cookie with `sameSite: lax`; only the main session cookie gets `strict`.

**SEC-02 — SSN audit: never in API response**
- Grep all route files for `ssn` in SELECT statements and response serialization.
- Verify that `workers` table SELECT queries explicitly exclude `ssn_encrypted` and `ssn_iv` from the response shape.
- Verify no `console.log`, `logger.info`, or audit log entry ever includes SSN plaintext or encrypted bytes.
- Add integration test: `GET /api/projects/:id/workers` response body — assert `JSON.stringify(body)` does not contain `ssn` key at any nesting level.

**SEC-03 — Rate limiting on auth endpoints**
- `POST /api/auth/login` should rate-limit to 10 attempts per IP per 15 minutes. Currently no rate limit → brute-force vulnerability.
- Use `express-rate-limit` (already in dependencies or add): `rateLimit({ windowMs: 15 * 60 * 1000, max: 10, message: { error: 'TOO_MANY_REQUESTS' } })` applied to `authRouter`.
- Same limit on `POST /api/auth/forgot-password` to prevent email enumeration via timing.

**Success Criteria:**
1. `Set-Cookie` header for session shows `SameSite=Strict`; OAuth flow still works via separate state cookie
2. `GET /api/projects/:id/workers` response contains no `ssn`, `ssn_encrypted`, or `ssn_iv` fields at any depth
3. No `ssn` value appears in any server log (grep log output during test run)
4. `POST /api/auth/login` with 11 requests in 1 minute from same IP returns 429 on the 11th
5. All existing auth tests still pass
...
### Phase 169: Mobile PWA Hardening — Offline Support + Push Notifications

**Goal**: Contractors use the app on mobile in areas with spotty coverage (job trailers, remote sites). Improve offline reliability: cache API responses for offline read; queue writes for background sync; add push notifications for deadline reminders and violation alerts (replacing email-only for mobile users).

**Priority**: P1 — field usability requirement; contractors complained about connectivity issues in pilot feedback

**Depends on**: Phase 167 (offline conflict resolution)

**Requirements**: PWA-01 through PWA-04

**UX Path:**
1. Worker opens app in a spotty coverage area. App shows "Offline mode — some data may be stale" banner at top.
2. Worker can view: dashboard (cached 5min ago), project detail, worker list, current payroll week — all from service worker cache.
3. Worker can clock in/out — action queued in IndexedDB. When connectivity returns, background sync fires automatically and flushes the queue.
4. Worker CANNOT create new payroll entries or export WH-347 offline (these require server computation); those buttons are grayed with "Requires connection" tooltip.
5. Push notifications: new notification permission request on first login (mobile only). Contractors opt-in to: "Deadline reminders", "Compliance violations", "Team activity". Notifications fire from the server via Web Push API.

**Implementation details:**
- Service worker: `src/client/sw.ts` — register cache strategy for `GET /api/dashboard`, `GET /api/projects`, `GET /api/workers` — cache-first with 5min TTL
- Background sync: `workbox-background-sync` plugin — queue failed POSTs to `/api/field/clock-in` and `/api/field/clock-out`
- Push: server-side `web-push` library; `POST /api/users/me/push-subscription` stores `PushSubscription` object; cron job fires `webpush.sendNotification()` for deadline reminders

**Success Criteria:**
1. App shows "Offline mode" banner when `navigator.onLine === false`
2. Dashboard, project list, and current payroll week render from cache while offline
3. Clock in/out while offline queues to IndexedDB; syncs within 30s of reconnection
4. WH-347 export button disabled with tooltip when offline
5. Push notification received within 60s of a compliance violation being created (when user has granted permission)
...
### Phase 170: Launch Gate — Smoke Tests, Copy Audit, Performance Baseline

**Goal**: Execute the full pre-launch checklist: automated smoke test suite against staging environment, a complete copy/content audit of every user-facing string (no dev placeholders, no typos, no "lorem ipsum"), a Lighthouse performance baseline on all 8 major pages, and a final security scan. Produce a Launch Readiness Certificate.

**Priority**: P0 — must pass before first paid customer

**Depends on**: All Phase 151-169 phases complete

**Requirements**: LAUNCH-01 through LAUNCH-06

**Smoke test suite (`tests/smoke/`):**
- `auth.smoke.ts`: register → login → logout → forgot password flow
- `project.smoke.ts`: create project → add worker → enter payroll week → download WH-347
- `compliance.smoke.ts`: enter under-wage payroll → assert violation fires → correct wage → assert cleared
- `mobile.smoke.ts`: FieldClockPage clock-in → supervisor approval → payroll entry uses approved hours
- `billing.smoke.ts`: click Upgrade → Stripe Checkout session created → callback sets plan → BillingPage shows correct plan
- All smoke tests run against `STAGING_URL` env var; CI blocks deployment on any failure

**Copy audit checklist:**
- `grep -r "TODO\|FIXME\|placeholder\|lorem ipsum\|demo\|test@\|Short answers" src/client/pages/` — zero results required
- All `<title>` tags on marketing pages have real content (not "React App")
- `ContactPage` "support email" is a real monitored inbox
- `PrivacyPolicyPage` and `TermsPage` have real legal content (not placeholder)
- All error messages are user-friendly (no raw exception strings, no "undefined" in toasts)

**Performance baseline (Lighthouse, targeting ≥ 90 on all):**
- `/` (LandingPage): Performance, SEO, Accessibility
- `/login` (LoginForm)
- `/dashboard` (DashboardPage)
- `/projects/:id` (ProjectDetailPage)
- `/projects/:id/payroll/:weekId` (PayrollWeekDetailPage)
- `/workers` / `/projects/:id/workers` (WorkersPage)
- `/reports` (GlobalReportsPage)
- `/compliance/map` (50-State Coverage Map)

**Security scan:**
- Run `npm audit` — zero high/critical vulnerabilities
- Run `npx retire.js` — no known-vulnerable client libraries
- Verify `Content-Security-Policy` header set on all responses
- Verify all cookies: `HttpOnly; Secure; SameSite=Strict`
- Verify no secrets in `git log --all --full-history -- '*.env'`

**Launch Readiness Certificate:**
- `LAUNCH_READINESS_MATRIX.md` (existing file) updated with final status column
- All P0 items: ✅ PASS
- All P1 items: ✅ PASS or 📋 DEFERRED with justification
- Performance baselines: all pages ≥ 90 Lighthouse
- Security scan: clean

**Success Criteria:**
1. All 6 smoke test files pass against staging environment
2. Copy audit grep returns zero matches for placeholder strings
3. Lighthouse ≥ 90 on all 8 pages for Performance and Accessibility
4. `npm audit` reports zero high or critical vulnerabilities
5. LAUNCH_READINESS_MATRIX.md shows all P0 rows as ✅ PASS
...
|-------|------|----------------|--------|-----------|
| 151. P0 Business Blockers | Stripe, registration, contact, email | 3/3 | ✅ Complete | 2026-05-18 |
| 152. P0 Compliance Calculation Fixes | DT hours, apprentice ratio, deduction guard | 0/TBD | Not started | - |
| 153. P0 Performance + DB Integrity | N+1 fix, migration journal, unique constraint | 0/TBD | Not started | - |
| 154. DashboardPage UX Redesign | Today lane, sparklines, activity feed | 0/TBD | Not started | - |
| 155. PayrollWeekDetailPage UX Overhaul | Readiness banner, section indicators, export gate | 0/TBD | Not started | - |
| 156. WorkersPage UX | Slide-over edit panel, CSV import polish | 0/TBD | Not started | - |
| 157. FieldClockPage UX | Crew status banner, worker grouping, GPS accuracy | 0/TBD | Not started | - |
| 158. GlobalReportsPage UX | Column sort, date filter, brand-navy token fix | 0/TBD | Not started | - |
| 159. Auth UX | Google OAuth, password strength, HCC# optional | 0/TBD | Not started | - |
| 160. OnboardingChecklist + DueSoonPanel | Required-only progress, urgency colors, snooze | 0/TBD | Not started | - |
| 161. QuickBooks 2-Way Write-Back | Certified payroll → QBO Journal Entry | 0/TBD | Not started | - |
| 162. Agency Portal / Reviewer Login | Read-only portal, comment thread, review stamp | 0/TBD | Not started | - |
| 163. Help Center + In-App Guidance | Glossary, guided tour, contextual tooltips | 0/TBD | Not started | - |
| 164. Supervisor Time-Punch Approval | Foreman submit → supervisor approve/reject | 0/TBD | Not started | - |
| 165. Sub-of-Sub + DBE Report | Tier 3 tracking, DBE participation PDF/CSV | 0/TBD | Not started | - |
| 166. SCA Support | Service Contract Act mode, SCA WD fetch | 0/TBD | Not started | - |
| 167. WA F700 Fix + Offline Conflicts | Real coordinates, conflict resolution UI | 0/TBD | Not started | - |
| 168. Security Hardening | sameSite:strict, SSN audit, rate limiting | 0/TBD | Not started | - |
| 169. Mobile PWA Hardening | Service worker cache, background sync, push notifications | 0/TBD | Not started | - |
| 170. Launch Gate | Smoke tests, copy audit, Lighthouse baseline, security scan | 0/TBD | Not started | - |
