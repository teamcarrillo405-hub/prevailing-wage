...
//
// VA DOLI Certified Payroll Report PDF generator using pdf-lib programmatic drawing.
//
// Unlike WH-347 which overlays a template, this form is drawn entirely from
// scratch using PDFDocument.create() — Virginia does not provide a state PDF template
...
//   Page 2 (ALWAYS dedicated): Statement of Compliance (Va. Code § 2.2-4360)
...
export interface VaPdfInput {
  ...
  project: {
    name: string;
    vaContractId: string | null;  // VA contract ID — goes in "Contract No." header field
    location: string;
...
// ── Column positions for the VA worker table ────────────────────────────────
...
function drawHeader(
  ...
  // Bold centered title
  const titleText = 'VIRGINIA CERTIFIED PAYROLL REPORT';
  const titleSize = 9;
  ...
  page.drawText(data.project.vaContractId ?? '', { x: 400, y, size: valueSize, font, color: black, maxWidth: 90 });
  y -= 8;
...
// ── Draw column headers for the VA worker table ──────────────────────────────

function drawTableHeaders(page: PDFPage, y: number, ctx: DrawCtx): number {
  const { boldFont, black } = ctx;
  const hSize = 5;

  page.drawText('Name/SSN', { x: VA_COL.nameSSN,  y, size: hSize, font: boldFont, color: black, maxWidth: 80 });
  page.drawText('Address',  { x: VA_COL.address,   y, size: hSize, font: boldFont, color: black, maxWidth: 65 });
  page.drawText('Class.',   { x: VA_COL.class,     y, size: hSize, font: boldFont, color: black, maxWidth: 63 });
  page.drawText('Mo',       { x: VA_COL.monSt,     y, size: hSize, font: boldFont, color: black, maxWidth: 13 });
  page.drawText('Tu',       { x: VA_COL.tueSt,     y, size: hSize, font: boldFont, color: black, maxWidth: 13 });
  page.drawText('We',       { x: VA_COL.wedSt,     y, size: hSize, font: boldFont, color: black, maxWidth: 13 });
  page.drawText('Th',       { x: VA_COL.thuSt,     y, size: hSize, font: boldFont, color: black, maxWidth: 13 });
  page.drawText('Fr',       { x: VA_COL.friSt,     y, size: hSize, font: boldFont, color: black, maxWidth: 13 });
  page.drawText('Sa',       { x: VA_COL.satSt,     y, size: hSize, font: boldFont, color: black, maxWidth: 13 });
  page.drawText('Su',       { x: VA_COL.sunSt,     y, size: hSize, font: boldFont, color: black, maxWidth: 13 });
  page.drawText('Rate',     { x: VA_COL.baseRate,  y, size: hSize, font: boldFont, color: black, maxWidth: 23 });
  page.drawText('H&W',      { x: VA_COL.hw,        y, size: hSize, font: boldFont, color: black, maxWidth: 18 });
...
function drawWorkerRow(
  page: PDFPage,
  entry: VaPdfInput['entries'][number],
  y: number,
  ctx: DrawCtx,
): number {
  const { font, boldFont, black } = ctx;
  ...
  page.drawText(fmtHours(entry.monSt), { x: VA_COL.monSt,    y, size: rowSize, font, color: black, maxWidth: 13 });
  page.drawText(fmtHours(entry.tueSt), { x: VA_COL.tueSt,    y, size: rowSize, font, color: black, maxWidth: 13 });
  page.drawText(fmtHours(entry.wedSt), { x: VA_COL.wedSt,    y, size: rowSize, font, color: black, maxWidth: 13 });
  page.drawText(fmtHours(entry.thuSt), { x: VA_COL.thuSt,    y, size: rowSize, font, color: black, maxWidth: 13 });
  page.drawText(fmtHours(entry.friSt), { x: VA_COL.friSt,    y, size: rowSize, font, color: black, maxWidth: 13 });
  page.drawText(fmtHours(entry.satSt), { x: VA_COL.satSt,    y, size: rowSize, font, color: black, maxWidth: 13 });
  page.drawText(fmtHours(entry.sunSt), { x: VA_COL.sunSt,    y, size: rowSize, font, color: black, maxWidth: 13 });

  // Rates and fringes
  page.drawText(fmtDollar(entry.baseRate),           { x: VA_COL.baseRate, y, size: rowSize, font, color: black, maxWidth: 23 });
  page.drawText(fmtDollar(entry.fringeHealthWelfare), { x: VA_COL.hw,       y, size: rowSize, font, color: black, maxWidth: 18 });
  page.drawText(fmtDollar(entry.fringePension),       { x: VA_COL.pension,  y, size: rowSize, font, color: black, maxWidth: 18 });
  page.drawText(fmtDollar(entry.fringeVacation),      { x: VA_COL.vacation, y, size: rowSize, font, color: black, maxWidth: 18 });
...
function drawStatementOfCompliance(
  ...
  // Paragraph 1 — statutory compliance certification referencing Va. Code § 2.2-4360
  const para1 =
    `I do hereby certify that the payroll records for the payroll period ending ${data.week.weekEndingDate} are correct and complete, and that the wage rate paid to each worker was not less than the applicable prevailing wage rate required under the Virginia Public Procurement Act, Va. Code § 2.2-4360 et seq.`;
  page.drawText(para1, {
    x: MARGIN,
    y,
    size: 7,
...
export async function fillVaCertifiedPayroll(data: VaPdfInput): Promise<Uint8Array> {
