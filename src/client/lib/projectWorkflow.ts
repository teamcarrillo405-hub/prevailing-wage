...
export function buildProjectWorkflowState(input: ProjectWorkflowInput) {
  ...
  const steps: ProjectWorkflowStep[] = [
    ...
    },
    {
      ...
      step: 4,
      label: 'Payroll',
      helper: 'Hours, deductions, compliance',
    ...
    },
    {
      ...
      helper: 'CPR requests and corrections',
      to: `${base}#subcontractors`,
      status: input.openCprItems > 0 ? 'warning' : 'clean',
    },
    {
      key: 'audit-packet',
      step: 6,
    ...
    },
    {
      ...
      helper: 'Final records',
      to: `${base}/reports`,
      status: submittedWeeks > 0 ? 'complete' : 'warning',
    },
  ];
